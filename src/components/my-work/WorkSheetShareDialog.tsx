...
import type { ShareableUser, WorkSheetShare } from '@/lib/my-work/types'
...
export function WorkSheetShareDialog({ sheetId, sheetName, open, onClose, onChanged }: Props) {
  ...
  useEffect(() => {
    if (!open) return
    setLoading(true)
    Promise.all([listWorkSheetShares(sheetId), getShareableUsers()])
      .then(([s, users]) => {
        setShares(s)
        setCandidates(users)
      })
  ...
  const handleShare = () => {
    ...
    start(async () => {
      try {
        const share = await shareWorkSheet(sheetId, selectedId, canEdit)
  ...
  const toggleEdit = (share: WorkSheetShare) => {
    start(async () => {
      try {
        const next = !share.can_edit
        await updateWorkSheetShare(sheetId, share.id, next)
  ...
  const handleRemove = (share: WorkSheetShare) => {
    start(async () => {
      try {
        await removeWorkSheetShare(sheetId, share.id)
